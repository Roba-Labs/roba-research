...
import { load_mujoco } from "../modules/mujoco/runtime.js";
import { OrbitControls } from "../vendor/three-orbit-controls.js";
import { GUI } from "../vendor/lil-gui.module.min.js";
import {
  downloadExampleScenes as downloadExampleScenesFolder,
  applyPositionFromBuffer as getPosition,
  applyQuaternionFromBuffer as getQuaternion,
  loadSceneFromUrl as loadSceneFromURL,
  setupGUI,
  randomStandardNormal as standardNormal,
  toMujocoCoords as toMujocoPos,
} from "../modules/mujoco/mujocoBridge.js";
import { DragStateManager } from "../modules/input/DragSessionManager.js";
import { InputManager } from "../modules/input/InputDeck.js";
import { createControllerForScene, createControllerByType } from "../modules/control/factory.js";
import { SimRecorder, SimPlayback } from "../modules/recording/SimulationArchive.js";
import { CanvasVideoRecorder } from "../modules/recording/CanvasCapture.js";
import { TelemetryOverlay } from "../modules/instrumentation/TelemetryOverlay.js";
import { WaypointManager } from "../modules/navigation/WaypointRegistry.js";
import { TargetSelector } from "../modules/navigation/TargetPicker.js";
import { PathSmoother } from "../modules/navigation/PathSmoother.js";
import { RoutePlanner } from "../modules/navigation/RoutePlanner.js";
import { RosBridge } from "../modules/robotics/ROSBridge.js";
import { SensorFusion } from "../modules/robotics/SensorFusion.js";
...
function mujocoPosToThree(x, y, z) {
  return new THREE.Vector3(x, z, -y);
}

// MuJoCo quat is (w,x,y,z); three wants (x,y,z,w) + frame rotation
function mujocoQuatToThree(qw, qx, qy, qz) {
...
export class MuJoCoDemo {
  constructor() {
    ...
    // Init recorders and telemetry
    this.simRecorder = new SimRecorder(this);
    this.simPlayback = new SimPlayback(this);
    this.playbackSpeed = 1.0;
    this.videoRecorder = new CanvasVideoRecorder(this.renderer.domElement);
    this.telemetry = new TelemetryOverlay(this.container);
    this.waypoints = new WaypointManager(this);
    this.targetSelector = new TargetSelector(this.scene, this.camera, this.renderer);
    this.pathSmoother = new PathSmoother(this);
    this.routePlanner = new RoutePlanner(this);
    this.ros = new RosBridge();
    ...
    this.hud.textContent = 'Initializing...';
  ...
  setActiveController(sceneName) {
    ...
    // Prefer explicit controller type if provided, otherwise auto
    const type = this.params?.controllerType || 'auto';
    this.controller = createControllerByType(type, this) || createControllerForScene(sceneName ?? this.params?.scene, this) || null;
    this.activeControllerType = this.controller?.type || null;

    // If ROS teleop is subscribed, wire incoming setpoints into PID
    if (this.ros) {
      this.ros.onTeleop = (sp) => {
        try {
          this.params.pidEnabled = true;
          this.pidTarget.alt = sp.alt;
          this.pidTarget.roll = sp.roll;
          this.pidTarget.pitch = sp.pitch;
          this.pidTarget.yawRateCmd = sp.yawRateCmd;
        } catch (e) { /* ignore */ }
      };
    }
  ...
  render(timeMS) {
    ...
    if (!this.params["paused"]) {
      ...
      if (timeMS - this.lastSimTimeMs > 35.0) {
        this.lastSimTimeMs = timeMS;
      }
      ...
      while (this.lastSimTimeMs < timeMS) {
        ...
        // Playback overrides: when active, drive sim from recorded frames
        if (this.simPlayback?.active) {
          this.simPlayback.stepOnce();
        }
        ...
        this.lastSimTimeMs += timestep * 1000.0;
      }
    ...
    const logCameraDebug = () => {
      if (this.frameCount % 30 === 0) {
        ...
        if (this.simulation.qpos && this.simulation.qpos.length >= 7) {
          ...
          const p = mujocoPosToThree(qpos[0], qpos[1], qpos[2]);
    ...
    if (this.cameraMode === 'track' && this.simulation?.cam_xpos) {
      ...
      const camP = mujocoPosToThree(p[addr+0], p[addr+1], p[addr+2]);
      ...
      if (!qCam && this.simulation.qpos?.length >= 3) {
        const look = mujocoPosToThree(this.simulation.qpos[0], this.simulation.qpos[1], this.simulation.qpos[2]);
    ...
    // --- FPV MODE ---
      ...
      const qpos  = this.simulation.qpos;
      const pos   = mujocoPosToThree(qpos[0], qpos[1], qpos[2]);
      const qBody = mujocoQuatToThree(qpos[3], qpos[4], qpos[5], qpos[6]);
    ...
    if (!this._hudCounter) this._hudCounter = 0;
    this._hudCounter++;
    if (this._hudCounter % 10 === 0) {
      const qpos = this.simulation?.qpos;
      const z = qpos && qpos.length >= 3 ? qpos[2] : 0;
      const fps = (this._lastFrameTimeMS !== undefined) ? (1000 / Math.max(1, timeMS - this._lastFrameTimeMS)).toFixed(0) : '---';
      this._lastFrameTimeMS = timeMS;
      const ctrlType = this.activeControllerType || (this.params?.controllerType ?? 'auto');
      this.hud.textContent = `FPS: ${fps}\nMode: ${this.cameraMode}\nCtrl: ${ctrlType}\nAlt: ${z?.toFixed ? z.toFixed(2) : z}`;
    }
    // Telemetry capture
    try {
      const qpos = this.simulation?.qpos;
      ...
      if (qpos && qpos.length >= 7) {
        const e = this.quatToRPY(qpos[3], qpos[4], qpos[5], qpos[6]);
        roll = e.roll || 0; pitch = e.pitch || 0;
      }
      // Apply Kalman filtering if enabled
      const fused = this.sensorFusion?.filter({ z, roll, pitch, yaw: 0 }) || { z, roll, pitch, yaw: 0 };
      const zF = fused.z, rollF = fused.roll, pitchF = fused.pitch;
      const ctrl0 = this.simulation?.ctrl?.[0] || 0;
      this.telemetry?.push(zF, rollF, pitchF, ctrl0);

      // Publish to ROS if enabled (use filtered z)
      if (this.ros?.params?.publishTelemetry && this.ros?.connected) {
        const pose = { x: qpos?.[0] || 0, y: qpos?.[1] || 0, z: zF, qx: qpos?.[4] || 0, qy: qpos?.[5] || 0, qz: qpos?.[6] || 0, qw: qpos?.[3] || 1 };
        this.ros.publishPoseStamped(pose);
      }
    } catch (e) { /* ignore */ }
    ...
    try { this.telemetry?.render(); } catch (e) { /* ignore */ }

    // Recording capture per frame (state only)
    try { this.simRecorder?.captureFrame(); } catch (e) { /* ignore */ }
  }
  ...
  updatePIDControl(nowMs) {
    ...
    if (this.frameCount && this.frameCount % LOG_FRAME_INTERVAL === 0) {
    ...
    this.pidTarget.yaw = wrapAngleToPi(
    ...
    const yawErr = wrapAngleToPi(this.pidTarget.yaw - yaw);
    ...
    for (let i = 0; i < 4 && i < this.thrustIndices.length; i++) {
      ...
      if (this.frameCount && this.frameCount % LOG_FRAME_INTERVAL === 0 && i === 0) {
        console.log(
